...
namespace ISAC017.api.Infraestructure.Services
{
    ...
    public class ManejadorResponse<T> : IManejadorResponse<T>
    {
        ...
        public async Task<T> RecibeMensajeSesion(string cadenaConexion, string nombreCola, string sesionId, int segundos, int intentos, int tiempo, string nombre)
        {
            ...
            if(messageSession != null) await messageSession.CloseAsync();
            if (sessionClient != null) await sessionClient.CloseAsync();
            if (serviceBusConnection != null) await serviceBusConnection.CloseAsync();
