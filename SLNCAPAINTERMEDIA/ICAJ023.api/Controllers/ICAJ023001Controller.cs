...
namespace ICAJ023.api.Controllers
{
    ...
    public class ICAJ023001Controller : ControllerBase
    {
        ...
        public async Task<ActionResult<APICAJ023001MessageResponse>> APICAJ023001(APICAJ023001MessageRequest parametrorequest)
        {
            ...
            {
                // Log
                Logger.FileLogger("APICAJ023001", "CONTROLADOR: El parámetro request es nulo.");
                return BadRequest();
            }
            ...
            try
            {
                ...
                Logger.FileLogger("APICAJ023001", "CONTROLADOR: Request Recibido : " + respuestaLog);
                ...
                for (int i = 1; i < vl_IntentosWS; i++)
                {
                    try
                    {
                        ...
                        {
                            respuesta = new APICAJ023001MessageResponse();
                            respuesta.SessionId = string.Empty;
                           // respuesta.StatusId = false;
                            respuesta.ErrorList = new List<string>();
                            respuesta.ErrorList.Add("ICAJ023:E000|EMPRESA NO EXISTE");
                        ...
                        if (ResuldatoHomologa != null && "OK".Equals(ResuldatoHomologa.DescripcionId))
                        {
                            ...
                            Logger.FileLogger("APICAJ023001", $"CONTROLADOR WS HOMOLOGACION: Código de empresa a enviarse a Dynamics es : {responseHomologa}");
                            cont = i;
                            break;
                        }
                    }
                ...
                Logger.FileLogger("APICAJ023001", $"CONTROLADOR WS HOMOLOGACION: Número de intentos realizados : {cont} En Ws Homologación,Tiempo Transcurrido : {diff} ms.");
                ...
                if (ResuldatoHomologa == null)
                {
                   ...
                   // respuesta.StatusId = false;
                    respuesta.ErrorList = new List<string>();
                    respuesta.ErrorList.Add("ICAJ023:E000|SERVICIO DE HOMOLOGACION NO DISPONIBLE");
                    return Ok(respuesta); // StatusCode(StatusCodes.Status204NoContent, respuesta);
                }
                ...
                respuesta = await manejadorReponseQueue.RecibeMensajeSesion(sbConenctionStringReceptar, nombrecolaresponse, sesionid, vl_Time, vl_Intentos, "APICAJ023001", vl_TimeOutResp);

                if (respuesta == null)
                {
                    respuestaLog = JsonConvert.SerializeObject(parametrorequest);
                    Logger.FileLogger("APICAJ023001", "CONTROLADOR: No se retorno resultado de Dynamics. ");
                    Logger.FileLogger("APICAJ023001", "CONTROLADOR: Request Enviado a Dynamics : " + respuestaLog);
                    respuesta = new APICAJ023001MessageResponse();
                    respuesta.SessionId = string.Empty;
                    //respuesta.StatusId = false;
                    respuesta.ErrorList = new List<string>();
                    respuesta.ErrorList.Add("ICAJ023:E000|NO SE RETORNO RESULTADO DE DYNAMICS");
                ...
                else
                {
                    ...
                    Logger.FileLogger("APICAJ023001", "CONTROLADOR: Response desde Dynamics: " + respuestaLog);
...
//              respuesta.StatusId = false;
                respuesta.ErrorList = new List<string>();
                respuesta.ErrorList.Add("ICAJ023:E000|NO SE RETORNO RESULTADO DE DYNAMICS"); //ex.Message
                Logger.FileLogger("APICAJ023001", "CONTROLADOR: Request Enviado a Dynamics: " + respuestaLog);
                Logger.FileLogger("APICAJ023001", "CONTROLADOR: Error por Exception: " + ex.Message);
