...
namespace ICRE007.api.Controllers
{
    ...
    public class ICRE007001Controller : ControllerBase
    {
        ...
        private IManejadorRequestQueue<APICRE007001MessageRequest> manejadorRequestQueue;
        private IManejadorResponseQueue2<APICRE007001MessageResponse> manejadorReponseQueue;
        ...
        public ICRE007001Controller(IManejadorRequestQueue<APICRE007001MessageRequest> _manejadorRequestQueue
            , IManejadorResponseQueue2<APICRE007001MessageResponse> _manejadorReponseQueue
        ...
        public async Task<ActionResult<APICRE007001MessageResponse>> APICRE007001(APICRE007001MessageRequest parametrorequest)
        {
            ...
            try
            {
                ...
                Logger.FileLogger("APICRE007001", "CONTROLADOR: Request Recibido : " + respuestaLog);
                ...
                for (int i = 1; i < vl_IntentosWS; i++)
                {
                    try
                    {
                        ...
                        if (ResuldatoHomologa != null && "OK".Equals(ResuldatoHomologa.DescripcionId))
                        {
                            ...
                            Logger.FileLogger("APICRE007001", $"CONTROLADOR WS HOMOLOGACION: Código de empresa a enviarse a Dynamics es : {responseHomologa}");
                            cont = i;
                            break;
                        }
                    }
                ...
                Logger.FileLogger("APICRE007001", $"CONTROLADOR WS HOMOLOGACION: Número de intentos realizados : {cont} En Ws Homologación,Tiempo Transcurrido : {diff} ms.");
                ...
                respuesta = await manejadorReponseQueue.RecibeMensajeSesion(sbConenctionStringReceptar, nombrecolaresponse, sesionid, vl_Time, vl_Intentos, "APICRE007001", vl_TimeOutResp);

                if (respuesta == null)
                {
                    respuestaLog = JsonConvert.SerializeObject(parametrorequest);
                    Logger.FileLogger("APICRE007001", "CONTROLADOR: No se retorno resultado de Dynamics. " );
                    Logger.FileLogger("APICRE007001", "CONTROLADOR: Request Enviado a Dynamics : " + respuestaLog);
                    respuesta = new APICRE007001MessageResponse();
                    respuesta.SessionId = string.Empty;
                    respuesta.StatusId = false;
                    respuesta.ErrorList = new List<string>();
                ...
                else
                {
                    ...
                    Logger.FileLogger("APICRE007001","CONTROLADOR: Response desde Dynamics: " + respuestaLog);
            ...
            catch (Exception ex)
            {
                ...
                Logger.FileLogger("APICRE007001", "CONTROLADOR: Request Enviado a Dynamics: " + respuestaLog);
                Logger.FileLogger("APICRE007001", "CONTROLADOR: Error por Exception: " + ex.Message);
