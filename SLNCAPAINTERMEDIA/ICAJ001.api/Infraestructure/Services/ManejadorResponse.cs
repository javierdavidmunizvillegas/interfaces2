...
namespace ICAJ001.api.Infraestructure.Services
{
    public interface IManejadorResponse<T>
    {
        Task<T> RecibeMensajeSesion(string cadenaConexion, string nombreCola, string sesionId, int segundos, int intentos, string nombre, int tiempo);
    }
    public class ManejadorResponse<T> : IManejadorResponse<T>
    {
        private static RegistroLog Logger = new RegistroLog();
        public async Task<T> RecibeMensajeSesion(string cadenaConexion, string nombreCola, string sesionId, int segundos, int intentos, string nombre, int tiempo)
        {
            int contador = 0;
