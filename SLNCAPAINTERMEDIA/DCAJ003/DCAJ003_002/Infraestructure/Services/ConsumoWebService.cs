...
namespace DCAJ003_002.Infraestructure.Services
{
    public interface IConsumoWebService<T>
     
    {
        Task<T> PostWebService(string uri, string method, string jsonData, int segundos, int intentos, string nombre);
    ...
    public class ConsumoWebService<T> : IConsumoWebService<T>
    {
        ...
        public async Task<T> PostWebService(string uri, string method, string jsonData, int segundos, int intentos, string nombre)
        {
            ...
            try
            {
                ...
                Logger.FileLogger(nombre, "RESPONSE WS: Número de  intentos realizados : " + Convert.ToString(contador) + $", Tiempo Transcurrido : {diff} ms.");
