...
namespace ILOG002_004.Infraestructure.Services
{
    public interface IManejadorHomologacion<T>
    {
        ...
        Task<T> GetHomologacion(string uri, string method, string urlRequest, int segundos, int intentos);
    }
    public class ManejadorHomologacion<T> : IManejadorHomologacion<T>
    {
        ...
        public async Task<T> GetHomologacion(string uri, string method, string urlRequest, int segundos, int intentos)
        {
            try
            {
                ...
                Logger.FileLogger("APILOG002004", "WS HOMOLOGACIÓN : Número de  intentos realizados : " + Convert.ToString(contador) + $", Tiempo Transcurrido : {diff} ms.");
