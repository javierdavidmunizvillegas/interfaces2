...
namespace IPRO007.api.Controllers
{
    ...
    public class IPRO007002Controller : ControllerBase
    {
        ...
        public async Task<ActionResult<APIPRO007002MessageResponse>> APIPRO007002(APIPRO007002MessageRequest parametrorequest)
        {
            ...
            {
                // Log
                Logger.FileLogger("APIPRO007002", "CONTROLADOR: El parámetro request es nulo.");
                return BadRequest();
            }
            ...
            try
            {
                ...
                Logger.FileLogger("APIPRO007002", "CONTROLADOR: Request Recibido : " + respuestaLog);
                ...
                for (int i = 1; i < vl_IntentosWS; i++)
                {
                    try
                    {
                        ...
                        {
                            respuesta = new APIPRO007002MessageResponse();
                        ...
                        if (ResuldatoHomologa != null && "OK".Equals(ResuldatoHomologa.DescripcionId))
                        {
                            ...
                            Logger.FileLogger("APIPRO007002", $"CONTROLADOR WS HOMOLOGACION: Código de empresa a enviarse a Dynamics es : {responseHomologa}");
                            cont = i;
                            break;
                        }
                    }
                ...
                Logger.FileLogger("APIPRO007002", $"CONTROLADOR WS HOMOLOGACION: Número de intentos realizados {cont} En Ws Homologación,Tiempo Transcurrido : {diff} ms.");
                ...
                respuesta = await manejadorReponseQueue.RecibeMensajeSesion(sbConenctionStringReceptar, nombrecolaresponse, sesionid, vl_Time, vl_Intentos,"APIPRO007002", vl_TimeOutResp);

                if (respuesta == null)
                {
                    respuestaLog = JsonConvert.SerializeObject(parametrorequest);
                    Logger.FileLogger("APIPRO007002", "CONTROLADOR: No se retorno resultado de Dynamics. ");
                    Logger.FileLogger("APIPRO007002", "CONTROLADOR: Request Enviado a Dynamics : " + respuestaLog);
                    respuesta = new APIPRO007002MessageResponse();
                    respuesta.SessionId = string.Empty;
                    respuesta.StatusId = false;
                    respuesta.ErrorList = new List<string>();
                ...
                else
                {
                    ...
                    Logger.FileLogger("APIPRO007002", "CONTROLADOR: Response desde Dynamics: " + respuestaLog);
            ...
            catch (Exception ex)
            {
                ...
                Logger.FileLogger("APIPRO007002", "CONTROLADOR: Request Enviado a Dynamics: " + respuestaLog);
                Logger.FileLogger("APIPRO007002", "CONTROLADOR: Error por Exception: " + ex.Message);
