...
namespace ICRE006.api.Infraestructura.Servicios
{
    ...
    public class ManejadorHomologacion<T> : IManejadorHomologacion<T>
    {
        ...
        public async Task<T> GetHomologacion(string uri, string method, string urlRequest, int segundos, int intentos, string nombre)
        {
            try
            {
                ...
                Logger.FileLogger(nombre, "SERVICIO RESPONSE - HOMOLOGACION : Número de  intentos realizados : " + Convert.ToString(contador) + $", Tiempo Transcurrido : {diff} ms.");
